...
class FirebaseMessangingRemoteDatasource {
  ...
  Future<void> initialize() async {
    ...
    const initializationSettingsAndroid = AndroidInitializationSettings('ic_permission');
    ...
    FirebaseMessaging.onMessage.listen((message) {
      C.showLog("-->${message.notification?.body ?? '--null--'}");
      C.showLog("-->${message.notification?.title ?? '--null--'}");
    });
  ...
  Future showNotification({int id = 0, String? title, String? body, String? payLoad}) async {
    return flutterLocalNotificationsPlugin.show(
      ...
      const NotificationDetails(
        android: AndroidNotificationDetails('com.example.flutter_absensi', 'app', importance: Importance.max),
